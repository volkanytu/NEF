...
using System.Linq;
using System.Text;
//using Tefal.ConsoleApp.SendCampaignActivityEmail.com.euromsg.auth;
//using Tefal.ConsoleApp.SendCampaignActivityEmail.com.euromsg.campaign;
//using Tefal.ConsoleApp.SendCampaignActivityEmail.sendlist;
using NEF.ConsoleApp.SendMarketingListEmail.com.euromsg.auth;
using NEF.ConsoleApp.SendMarketingListEmail.com.euromsg.campaign;
using NEF.ConsoleApp.SendMarketingListEmail.sendlist;
...
namespace NEF.ConsoleApp.SendMarketingListEmail
{
    public class EmailFunctions
    {
        ...
        public void UpdateEmailActivity(EmailDetail emaildetail, SqlDataAccess sda)
        {
            try
            {
                ...
                SqlParameter[] parameters = new SqlParameter[]{
                                                ...
                                                new SqlParameter("@GsId", emaildetail.Key)
        ...
        public string BuildXml(DataRow drActivity)
        {
            ...
            {
                logMe.Log("SendMailIntegration - Execute", ex, TEMPEventLog.EventType.Exception, "list", drActivity["CAID"].ToString());
            }
        ...
        public DataTable GetCustomerList(SqlDataAccess sda,  Guid ListId)
        {
            ...
            try
            {
                ...
                string queryList = @"
                                  SELECT
                                      ...
                                      C.new_gsid GSID,
                                      CASE 
                                          WHEN C.EMailAddress1 is not null THEN C.EMailAddress1
                                          ELSE '-1'
                                      END as EM,
            ...
            {
                logMe.Log("EmailFunctions - GetCustomerList", ex, TEMPEventLog.EventType.Exception, "list", ListId.ToString());
            }
        ...
        public string BuildMailList(DataTable customerList)
        {
            ...
            try
            {
                ...
                {
                    mailList = mailList + dr["GSID"].ToString().Replace("|", "").Replace(Environment.NewLine, "").Replace(Convert.ToChar(0x0).ToString(), "") + "|" +
        ...
        public string BuildDataFile(string customerList,string subject)
        {
            ...
            // build header
            line = "KEY_ID|EMAIL|NAME|SURNAME|CARDNO";
            // write header
