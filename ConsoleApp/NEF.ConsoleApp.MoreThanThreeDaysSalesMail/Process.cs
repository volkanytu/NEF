    ...
    class Process
    {
        internal static void Execute(SqlDataAccess sda, IOrganizationService service)
        {
            ...
            string sqlQuery = @"SELECT
                                    ...
                                    Q.new_salesofficeidName AS [Ofis],
                                    CONVERT(varchar, CAST(Q.new_exchangerate AS money), 1) AS [Döviz Kuru],                                   
                                    ...
                                    WHERE
                                    CAST(Q.new_contractprocessdate as date)<=@date
                                    AND
            ...
            DataTable dt = sda.getDataTable(sqlQuery, new SqlParameter[] { new SqlParameter("date", DateTime.Now.AddDays(-3).ToUniversalTime().Date) });
            #endregion SQL QUERY
        ...
        public static void SendMailToResourceImpoter(DataTable dt_unprocessed, IOrganizationService service)
        {
            try
            {
                ...
                email["subject"] = "3 Günden Fazla Sözleşme İmzalanan Satışlar";
