...
using System.Linq;
using System.Text;
//using NEF.ConsoleApp.SendCampaignActivitySms.com.euromsg.Report;
//using NEF.ConsoleApp.SendCampaignActivitySms.com.euromsg.Auth;
//using NEF.ConsoleApp.SendCampaignActivitySms.com.euromsg.Campaign;
using NEF.ConsoleApp.SendCampaignActivitySms.com.euromsg.Auth.live;
using NEF.ConsoleApp.SendCampaignActivitySms.com.euromsg.Campaign.live;
using NEF.ConsoleApp.SendCampaignActivitySms.com.euromsg.Report.live;
    ...
    public class SmsFunctions
    {
        ...
        public string BuildXml(DataRow drActivity)
        {
            ...
            try
            {
                ...
                campaignXml.Append("<CAMP_NAME>").Append(drActivity["CS"].ToString()).Append("</CAMP_NAME>");
                ...
                campaignXml.Append("<DELIVERY_START_DATE>").Append(Convert.ToDateTime(drActivity["CAS"]).ToString("yyyy-MM-dd HH:mm:ss")).Append("</DELIVERY_START_DATE>");
                ...
                campaignXml.Append("<ORIGINATOR>").Append("EUROMSG").Append("</ORIGINATOR>");
                campaignXml.Append("<UNIQUE_SMS>").Append("Y").Append("</UNIQUE_SMS>");
                campaignXml.Append("<MESSAGE>").Append("<![CDATA[" + drActivity["MSG"] != null ? drActivity["MSG"].ToString() : string.Empty + "]]> ").Append("</MESSAGE>");
                campaignXml.Append("<ALTERNATE_MESSAGE>").Append("<![CDATA[" + string.Empty + "]]>").Append("</ALTERNATE_MESSAGE>");
        ...
        public DataTable GetCustomerList(SqlDataAccess sda, Guid campaignActivityId, Guid ListId)
        {
            ...
            {
                logMe.Log("SmsFunctions - GetCustomerList", ex, TEMPEventLog.EventType.Exception, "new_campaignactivity", campaignActivityId.ToString());
            }
        ...
        public string BuildSmsList(DataTable customerList)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in customerList.Rows)
                {
                    ...
                    gsm = gsm.Replace("+", "");
                    gsm = gsm.Replace("-", "");
                    gsm = gsm.Substring(2, 10);
                    gsm = gsm.ToString().Substring(0, 3) + "." + gsm.ToString().Substring(3);

                    smsList = smsList +
                         //dr["CID"].ToString().Replace("|", "").Replace(Environment.NewLine, "").Replace(Convert.ToChar(0x0).ToString(), "") + "|"  +
                         dr["EM"].ToString().Replace("|", "").Replace(Environment.NewLine, "").Replace(Convert.ToChar(0x0).ToString(), "") + "|" +
                    ...
                    smsDetails.Add(new SmsDetail
                    {
                        ...
                        Description = dr["ST"].ToString(),
                        Subject = dr["SJ"].ToString(),
                    });
                }
                smsList += smsList.Trim();
        ...
        public string BuildDataFile(string customerList)
        {
            ...
            // build header
            line = "EMAIL|GSM_NO|NAME|SURNAME";
            // write header
