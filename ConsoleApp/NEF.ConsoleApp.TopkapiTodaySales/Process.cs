...
namespace NEF.ConsoleApp.TopkapiTodaySales
{
    class Process
    {
        internal static void Execute(SqlDataAccess sda, IOrganizationService service)
        {
            ...
            string sqlQuery = @"SELECT
									...
									AND
									sm.AttributeName='new_prepaymenttype'
									AND 
									sm.AttributeValue = Q.new_prepaymenttype) AS 'Ön Ödeme Türü'
                                    FROM
                                    Quote AS Q (NOLOCK)
                                    INNER JOIN
            ...
            DataTable dt = sda.getDataTable(sqlQuery, new SqlParameter[] { new SqlParameter("date", DateTime.Now.ToUniversalTime().Date) });
            #endregion SQL QUERY
        ...
        public static void SendMailToResourceImpoter(DataTable dt_unprocessed, string totalCount, decimal TL, decimal Dolar, decimal Euro, IOrganizationService service)
        {
            try
            {
                ...
                Entity toParty = new Entity("activityparty");
                toParty["addressused"] = "MII@isgyo.com.tr";
                Entity toParty2 = new Entity("activityparty");
                toParty2["addressused"] = "satiskiralama@isgyo.com.tr";
                Entity toParty3 = new Entity("activityparty");
                toParty3["addressused"] = "gulfem.tandogan@isgyo.com.tr ";
                
                Entity ccParty = new Entity("activityparty");
                ccParty["addressused"] = "erkan.ozvar@nef.com.tr";

                Entity email = new Entity("email");
                email["to"] = new Entity[] { toParty, toParty2, toParty3 };
