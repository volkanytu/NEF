...
var ActivityDetailHelper = {
    ...
    "GetDisplayLocationAttachment": function (activityId, callbackFunction) {
        var jData = {};
        jData.activityId = activityId;
        var jSonData = JSON.stringify(jData);

        $.ajax({
            url: CustomServiceUrl + "/GetDisplayLocationAttachment",
            async: true,
            data: jSonData,
            dataType: "json",
            contentType: "application/json;",
                ...
                if (data != null) {
                    ...
                    if (data.Success) {
                        if (data.ReturnObject != null) {
                            $("#ShowDisplayLocationFiles").show();
                            callbackFunction(data.ReturnObject);

                        } else {
                            $("#ShowDisplayLocationFiles").hide();
                        }
                    }
                    else {
                        parent.IndexHelper.ToastrShow(false, data.Result, "Yer Gösterme Belgesi Belgeleri Bilgileri");
                    }
                }
                else {
    ...
    "DownloadDisplayLocationFile": function () {
        var activityid = ActivityDetailHelper.ActivityId;
        var winOpener = window.open("upload.ashx?operation=8&id=" + activityid);
    },
...
function mainController($scope) {
    ...
    ActivityDetailHelper.GetActivityInfo(function (e) {
        ...
        $scope.$apply(function () {
            ...
            ActivityDetailHelper.GetDisplayLocationAttachment($scope.Activity.ActivityId, function (e) {
                $scope.$apply(function () {
                    $scope.GetDisplayLocationAttachment = e;
                });
            });
    ...
    $scope.DownloadDisplayLocationFile = function () {
        ActivityDetailHelper.DownloadDisplayLocationFile();
    }
