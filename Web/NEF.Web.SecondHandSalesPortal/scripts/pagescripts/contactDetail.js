...
var ContactDetailHelper = {
    ...
    "GetCustomerRentals": function (callbackFunction) {
        var jData = {};
        ...
        $.ajax({
            url: CustomServiceUrl + "/GetCustomerRentals",
            async: true,
            dataType: "json",
            contentType: "application/json;",
            type: "POST",
            data: jSonData,
            beforeSend: function () {
                Metronic.blockUI({ target: $("#plBodySales"), iconOnly: false, boxed: true, message: "Kiralamalar getiriliyor..." });
            },
            complete: function () {
                Metronic.unblockUI($("#plBodySales"));
            },
    ...
    "GetCustomerSecondHands": function (callbackFunction) {
        var jData = {};
        jData.contactId = ContactDetailHelper.ContactId;
        jData.systemUserId = parent.IndexHelper.UserId;
        jData.accountId = "";
        ...
        $.ajax({
            url: CustomServiceUrl + "/GetCustomerSecondHands",
            async: true,
            dataType: "json",
            contentType: "application/json;",
            type: "POST",
            data: jSonData,
            beforeSend: function () {
                Metronic.blockUI({ target: $("#plBodySales"), iconOnly: false, boxed: true, message: "2. El Satışlar getiriliyor..." });
            },
            complete: function () {
                Metronic.unblockUI($("#plBodySales"));
            },
    ...
    "UploadImageToCrm": function () {
        try {
            ...
            var name = "CRM_NFZ_" + $("#lblImage").attr('fileName');
            var conId = ContactDetailHelper.ContactId;
            debugger;
            $.ajax({
                ...
                beforeSend: function () {
                    Metronic.blockUI({ target: $("#plBodyUserInfo"), iconOnly: false, boxed: true, message: "Nüfus cüzdanı yükleniyor..." });
                    $("#btnUpload").hide();
                },
                complete: function () {
                    Metronic.unblockUI($("#plBodyUserInfo"));
                ...
                success: function (data) {
                    if (data != null) {
                        if (data == true) {
                            parent.IndexHelper.ToastrShow(true, "Nüfus cüzdanı başarıyla yüklendi.", "Dosya Yükleme");
...
app.controller('ctrlTest', ['$scope', function ($scope) {
    ...
    $scope.UploadCrm = function () {
        ContactDetailHelper.UploadImageToCrm();
    }
            ...
            if (results.Success) {
                $scope.$apply(function () {
                    ...
                    if ($scope.contact != null) {
                        ...
                        if ($scope.contact.Annotation != null) {
                            $("#lblImage").show();
                            $("#btnSelectPhoto").hide();
                        } else {
                            $("#btnSelectPhoto").show();
    ...
    $scope.Save = function () {
        ...
        //var guarantorId = $("#txtGuarantor").attr("crmid");

        //if (guarantorId != null && guarantorId != undefined && guarantorId != "") {

        //    if ($scope.contact.GuaContact == null)
        //        $scope.contact.GuaContact = {};
        //    $scope.contact.GuaContact.Id = $("#txtGuarantor").attr("crmid");
        //    $scope.contact.GuaContact.Name = $("#txtGuarantor").val();
        //    $scope.contact.GuaContact.LogicalName = "contact";
        //}
    ...
    $scope.OpenRentalPage = function (rentalId) {
        if (parent.IndexHelper.UserType != UserTypes.CallCenter) {
            parent.IndexHelper.ShowDialog("<iframe src='editrental.html?rentalid=" + rentalId + "' style='width:100% !important; border:none;height:450px;padding-left:0px !important;padding-right:0px !important;overflow-y:scroll !important;overflow-x: hidden !important;' scrolling='auto'></iframe>", "<i class='icon-flag'></i> Satış Kaydı", "modal90");
        }
    }

    $scope.OpenSecondHandPage = function (secondhandId) {
        if (parent.IndexHelper.UserType != UserTypes.CallCenter) {
            parent.IndexHelper.ShowDialog("<iframe src='editSecondHand.html?secondhandid=" + secondhandId + "' style='width:100% !important; border:none;height:450px;padding-left:0px !important;padding-right:0px !important;overflow-y:scroll !important;overflow-x: hidden !important;' scrolling='auto'></iframe>", "<i class='icon-flag'></i> Satış Kaydı", "modal90");
        }
    }
    ...
    $scope.RefreshSecondHands = function () {
        GetCustomerSecondHands();
    }
    ...
    //$scope.RemoveGuaContact = function () {
    //    $scope.contact.GuaContact = null;
    //    $("#txtGuarantor").attr("crmid", null);
    //    $("#txtGuarantor").val(null);
    //}
    ...
    $scope.OpenGuaContactSearchPage = function () {
        parent.IndexHelper.ShowDialog("<iframe src='contactsearch.html?SearchType=1' style='width:100% !important; border:none;height:650px;padding-left:0px !important;padding-right:0px !important;overflow-y:scroll !important;overflow-x: hidden !important;' scrolling='auto'></iframe>", "<i class='icon-calendar'></i> Kişi Arama", "modal90");
    };
    ...
    function GetCustomerRentals() {
        ContactDetailHelper.GetCustomerRentals(function (e) {
            $scope.$apply(function () {
                if (e.Success) {
                    $scope.rentals = e.ReturnObject;
    ...
    function GetCustomerSecondHands() {
        ContactDetailHelper.GetCustomerSecondHands(function (e) {
            $scope.$apply(function () {
                if (e.Success) {
                    $scope.secondhands = e.ReturnObject;
    ...
    function GetContactAttachment(annotId) {

        var URL = 'http://fenixcrm.nef.com.tr' + '/notes/edit.aspx?id=' + annotId;
        $.get(URL, function (data) {
            var WRPCTokenElement = $(data).find("[WRPCTokenUrl]");
            if (WRPCTokenElement) {
                var WRPCTokenUrl = WRPCTokenElement.attr("WRPCTokenUrl");
                if (WRPCTokenUrl) {
                    URL = "/Activities/Attachment/download.aspx?AttachmentType=5&AttachmentId={" + annotId + "}&IsNotesTabAttachment=undefined" + WRPCTokenUrl;
                    window.open(URL);
                }
            }
        });
