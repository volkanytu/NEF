...
{
    public class MinistryBankCreditHelper
    {
        IOrganizationService service;

        internal string ExecuteDocumentWritenRecordForm(Guid productId, string Path)
        {
            string folder = MinistryBankCreditHelper.CreateFolder(productId, Path);
            ...
            byte[] bytes = DocumentMerge.WordDokumanOlustur(Path + "DocumentMerge\\Templates\\MinistryBankCredit.docx", (DataSet)null, dictionary1);
            string path1 = Path + "DocumentMerge\\Document\\" + folder + "\\MinistryBankCredit.docx";
