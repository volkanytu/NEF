...
{
    public class DeliveryCertificateHelper
    {
        IOrganizationService service;

        internal string ExecuteDocumentWritenRecordForm(Guid productId, string Path)
        {
            string folder = DeliveryCertificateHelper.CreateFolder(productId, Path);
            ...
            byte[] bytes = DocumentMerge.WordDokumanOlustur(Path + "DocumentMerge\\Templates\\DeliveryCertificate.docx", (DataSet)null, dictionary1);
            string path1 = Path + "DocumentMerge\\Document\\" + folder + "\\DeliveryCertificate.docx";
