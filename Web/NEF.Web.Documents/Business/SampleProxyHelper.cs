...
{
    public class SampleProxyHelper
    {
        IOrganizationService service;

        internal string ExecuteDocumentWritenRecordForm(Guid productId, string Path)
        {
            string folder = SampleProxyHelper.CreateFolder(productId, Path);
            ...
            dictionary1.Add("KrediBanka", banka);
            ...
            byte[] bytes = DocumentMerge.WordDokumanOlustur(Path + "DocumentMerge\\Templates\\SampleProxy.docx", (DataSet)null, dictionary1);
            string path1 = Path + "DocumentMerge\\Document\\" + folder + "\\SampleProxy.docx";
